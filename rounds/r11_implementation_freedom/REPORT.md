...
prediction below were written down in `rounds/r11_implementation_freedom/THEORY.md` **before any model in this round
was trained**; this report says which of them survived.

Numbers from `rounds/r11_implementation_freedom/tables.md`, per-run values in `rounds/r11_implementation_freedom/results10.json`, weights in
`rounds/r11_implementation_freedom/models/`, figures `fig1_routing.png`, `fig2_factorization.png`, `fig3_boundary.png`.
Reproduce with `rounds/r11_implementation_freedom/run.py --exp all --steps 25000`.
...
`rounds/r11_implementation_freedom/THEORY.md` §2.
...
only; `tables.md` and `fig1_routing.png` average over all seeds of each condition, so their
...
* Training the grid as one stacked model is worth it: 4× throughput and, more usefully, the
  ...
  rebuilds every measurement from `rounds/r11_implementation_freedom/models/` in 4 minutes).
