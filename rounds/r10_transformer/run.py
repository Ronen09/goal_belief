...
invariants on a 2-layer pre-LN causal transformer, with and without the final LayerNorm.
    ...
    .venv/bin/python rounds/r10_transformer/run.py [--quick] [--jobs 8] [--out results9]
"""
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--quick", action="store_true"); ap.add_argument("--out", default="rounds/r10_transformer"); ap.add_argument("--jobs", type=int, default=4)
    ...
    with ProcessPoolExecutor(max_workers=args.jobs, mp_context=mp.get_context("spawn")) as ex:   # CUDA cannot be forked
