...
`rounds/r14_window_carry/THEORY.md`, committed (`1b3fcf1`) **before any model was trained**. Model:
`goalgeo/wtfm.py`. Numbers from `rounds/r14_window_carry/tables.md`, per-model values in `results13.json`,
weights in `rounds/r14_window_carry/models/`, figure `fig1_window.png`. Reproduce: `rounds/r14_window_carry/run.py`
(31 min on CPU workers), then `rounds/r14_window_carry/tables.py`.
