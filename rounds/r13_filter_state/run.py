"""TASK12 (round 13): the full filter state (rounds/r13_filter_state/THEORY.md).
...
bottleneck models.
    ...
    .venv/bin/python rounds/r13_filter_state/run.py                 # ~15 min, CPU workers
    .venv/bin/python rounds/r13_filter_state/run.py --measure-only
"""
...
R12 = Path("rounds/r12_hidden_goal/models")
...
def main():
    ...
    ap.add_argument("--out", default="rounds/r13_filter_state")
    ...
    conv = {r["name"]: r.get("converged") for r in json.load(open("rounds/r12_hidden_goal/results11.json"))["runs"]}
    for r in recs:
        if r.get("behaviour"):
