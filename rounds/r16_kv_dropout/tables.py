"""TASK15 (round 16): tables, prediction checks and figure from rounds/r16_kv_dropout/results15.json.

    .venv/bin/python rounds/r16_kv_dropout/tables.py
"""
...
OUT = Path("rounds/r16_kv_dropout")
PS = (0.0, 0.5, 0.75, 0.9, 0.97, 1.0)
TS = (4, 8, 16, 23)
RP = json.load(open("rounds/r15_prior_vs_recompute/recompute_prediction.json"))
...
def tables(d: D) -> str:
    ...
    if fine:
        L += ["## Follow-up (not pre-registered): the finer dropout grid, merged", "",
              ...
              "transition lies between p = 0 and 0.5 (`rounds/r16_kv_dropout/fine/`). Prior weight at t = 16.", "",
