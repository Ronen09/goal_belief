...
Run date: 2026-09-23. Brief: `rounds/r16_kv_dropout/BRIEF.md`. Theory and 7 predictions: `rounds/r16_kv_dropout/THEORY.md`,
committed (`57b3e10`) **before any model was trained**. A finer dropout grid was added afterwards
and is reported separately as a follow-up (`rounds/r16_kv_dropout/fine/`). Numbers from `rounds/r16_kv_dropout/tables.md`,
per-model values in `results15.json` and `fine/results15.json`, weights in `models/`, figure
`fig1_incentive.png`. Reproduce: `rounds/r16_kv_dropout/run.py` (30 min), then
`rounds/r16_kv_dropout/run.py --out rounds/r16_kv_dropout/fine --ps 0.05 0.1 0.2 0.3 --plain-ps 0.1 0.25`, then
`rounds/r16_kv_dropout/tables.py`.
