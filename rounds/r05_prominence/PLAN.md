...
**Architecture:** A parametrised HMM family (`goalgeo/hmm4.py`) with vectorised inference, a weighted sequential trainer with in-memory checkpoints (`train_weighted` in `goalgeo/seqmodels.py`), a measurement module (`goalgeo/prominence.py`) that returns one flat dict per checkpoint, a runner (`rounds/r05_prominence/run.py`) that runs conditions in parallel processes and writes tables/JSON, and figures (`rounds/r05_prominence/plots.py`).

**Tech Stack:** Python 3.12, numpy, scipy, torch 2.14 (ROCm, CPU used with `--jobs`), matplotlib, pytest. No sklearn, no pandas.

**Spec:** `rounds/r05_prominence/DESIGN.md`
...
- Outputs in `rounds/r05_prominence/`: `tables.md`, `results4.json`, `fig1_info_vs_prominence.png`, `fig2_prominence_vs_cost.png`, `fig3_gradient_vs_prominence.png`, `fig4_dynamics.png`, `fig5_delay.png`, `REPORT.md`.
...
### Task 4: Runner `rounds/r05_prominence/run.py` (conditions, parallel runs, analysis, tables)

**Files:**
- Create: `rounds/r05_prominence/run.py`
...
- Produces: `rounds/r05_prominence/results4.json` with `{"runs": [run...], "conditions": {...}, "regression": {...}, "dynamics": {...}, "grid_ran": bool}`; each run = `{"cond": {r, delta, k, lam}, "sweeps": [...], "seed", "init": measure dict, "final": measure dict, "integrated": {"G_rel_int","Gpar_rel_int","G_imm_int","Gpar_imm_int"}, "forget": {...}, "cost": lam*per_step, "dynamics": {step: measure dict}, "loss_hist": [...]}`; `rounds/r05_prominence/tables.md`.

- [ ] **Step 1: Write the script**

```python
# rounds/r05_prominence/run.py
...
with checkpoints, and regresses prominence on decodability, forgetting cost and gradients.
    ...
    .venv/bin/python rounds/r05_prominence/run.py [--quick] [--jobs 6] [--grid] [--out results4]
"""
from __future__ import annotations
...
def main():
    ...
    ap.add_argument("--quick", action="store_true"); ap.add_argument("--out", default="rounds/r05_prominence")
...
Run: `.venv/bin/python rounds/r05_prominence/run.py --quick --jobs 4 --out /tmp/claude-1000/-home-ronenrr-Documents-goals/134cc8ee-4117-408e-b170-0397caf7ba60/scratchpad/q4`
Expected: prints one line per run (8 conditions + 1 matched = 9 runs), "figures skipped" (Task 5 not yet done), writes `tables.md` and `results4.json`. Inspect `tables.md` for finite numbers.
...
### Task 5: Figures `rounds/r05_prominence/plots.py`

**Files:**
- Create: `rounds/r05_prominence/plots.py`
...
```python
# rounds/r05_prominence/plots.py
...
Run: `.venv/bin/python rounds/r05_prominence/run.py --quick --jobs 4 --out <scratchpad>/q4 && ls <scratchpad>/q4/*.png`
...
**Files:**
- Create: `rounds/r05_prominence/REPORT.md`
...
Run: `.venv/bin/python rounds/r05_prominence/run.py --jobs 8 2>&1 | tee rounds/r05_prominence/run_log.txt` (background; expect 40–90 min).
Expected: ~140 runs (more if the grid is triggered), `rounds/r05_prominence/tables.md`, `rounds/r05_prominence/results4.json`, five figures.
...
- [ ] **Step 3: Write `rounds/r05_prominence/REPORT.md`**

Sections: setup (HMM family, model, run date, wall time), the five sweeps with the table columns that matter (P_metric, P_metric_pre, belief R², branch accuracy, ∫G_∥, cost, KL), the regression and leave-one-sweep-out numbers, the dynamics ordering, controls 1–5, the reading against the eight success criteria and outcomes A–D of rounds/r05_prominence/BRIEF.md, and files.

- [ ] **Step 4: Update README.md and memory**

Add a round 5 row (rounds/r05_prominence/BRIEF.md → spec, `rounds/r05_prominence/run.py --jobs 8`, `rounds/r05_prominence/REPORT.md`), the setup command, layout rows for `hmm4.py`, `prominence.py`, `plotting4.py`, and the new test count. Update the memory file with the headline results and how to rerun.

- [ ] **Step 5: Final verification**

Run: `.venv/bin/python -m pytest -q`
Expected: all pass. Confirm every file listed in Global Constraints exists in `rounds/r05_prominence/`.
