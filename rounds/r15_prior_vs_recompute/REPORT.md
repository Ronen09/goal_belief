...
Run date: 2026-09-23. Brief: `rounds/r15_prior_vs_recompute/BRIEF.md`. Theory and 9 predictions: `rounds/r15_prior_vs_recompute/THEORY.md`,
committed (`c2fbd70`) **before any model was trained**. Code: `goalgeo/kvprior.py`. Numbers from
`rounds/r15_prior_vs_recompute/tables.md` (§1–4 as pre-registered, §5 post hoc), per-model values in
`results14.json`, weights in `rounds/r15_prior_vs_recompute/models/`, figure `fig1_prior_vs_recompute.png`.
Reproduce: `rounds/r15_prior_vs_recompute/run.py` (11 min on one GPU), `rounds/r15_prior_vs_recompute/recompute.py`, then
`rounds/r15_prior_vs_recompute/tables.py`.
