...
Date: 2026-09-17. Source of requirements: `TASK.md`.
...
results report (figures + numbers) covering Phases 1–8 of `TASK.md`, and the
first two items of Phase 9 (stochastic transitions).

## 2. Decisions and assumptions (things `TASK.md` leaves open)
...
| Optimal policy | Uniform distribution over the argmax actions of `Q_g`. | `TASK.md` asks for `π*_g(a|s)` as a distribution; ties are common on grids. |
...
- `ρ(g|s,a)` = `(1−γ)·1[s=g] + γ Σ_{s'} P(s'|s,a) D_g[s', g]` — state-action occupancy as in `TASK.md`.
...
scripts/run_all.py   runs every phase, writes results/<env>/... and results/REPORT.md
tests/               pytest: exact analytic checks for occupancy, geometry, model
```

Results are written under `results/` as JSON plus PNG figures; the report
`results/REPORT.md` is generated by the run script and then edited by hand
with interpretation.
