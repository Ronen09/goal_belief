...
to it). Numbers from `results9/tables9.md`, per-run values in
`results9/results9.json`, the cut diagnosis in `results9/followup_cuts.json`,
weights in `results9/models/`. Reproduce with `scripts/run_task9.py --jobs 4`
then `scripts/task9_followup.py`.
...
position is not, and `docs/task7_theory.md` §4 now states the condition. Which
