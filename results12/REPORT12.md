...
Run date: 2026-09-23. Brief: `TASK12.md`. Theory and 10 predictions: `docs/task12_filter_theory.md`,
...
(`results12/models/`). Numbers from `results12/tables12.md`, per-model values in `results12.json`,
figures `fig1_interventions.png`, `fig2_hierarchy.png`, `fig3_bottleneck.png`. Reproduce:
`scripts/run_task12.py` (7 min on 96 CPU workers), then `scripts/task12_tables.py`.
...
* **At every level, the network's histories diverge by the Bayes amount.** Equal b: the future
  ...
  magnitude below random pairs. `act_soft` and `next_obs` show the same pattern (tables12.md §3).
