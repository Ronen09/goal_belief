...
Run date: 2026-09-17. All numbers below are means over seeds from `results/tables.md`
(5 seeds for the base grid, 3 for the discriminating and stochastic environments).
Design and assumptions: `docs/superpowers/specs/2026-09-17-goal-occupancy-geometry-design.md`.
Reproduce with `.venv/bin/python scripts/run_all.py` (about 6 minutes on CPU).
...
portal 0.79, twins 0.32; maps in `results/phase6/env_*.png`). Three-way RDM
...
- `results/tables.md`: every number, all layers, both encodings.
- `results/base/*.json`, `results/phase6/phase6.json`, `results/stochastic/phase9.json`: raw per-seed values.
- `results/run_log.txt`: console log of the run.
