...
Run date: 2026-09-23. Brief: `TASK11.md` (claim 2 and the three tests that followed it). Theory,
cut analysis and the 14 predictions are in `docs/task11_causal_theory.md`, committed (`56b0d1f`)
**before any intervention was run**. This part uses the round-12 models (K = 4, main grid, 4
seeds per cell, 64 models); nothing is retrained. Numbers from `results11/causal/tables_causal.md`,
per-model series in `causal.json`, pairs in `pairs_*.npz`, figures `fig4_transplant.png`,
`fig5_equal_belief.png`, `fig6_ladder.png`. Reproduce: `scripts/run_task11_causal.py` (4 min on
64 CPU workers), then `scripts/task11_causal_tables.py`.
