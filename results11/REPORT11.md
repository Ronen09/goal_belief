...
Run date: 2026-09-23. Brief: `TASK11.md` (claim 1 of three; the brief did not include claims 2
and 3). Theory and the eleven predictions in `docs/task11_theory.md` were committed (`9cdcad4`)
**before any model of this round was trained**. Numbers from `results11/tables11.md`, per-model
values in `results11/results11.json`, weights in `results11/models/`, figures
`fig1_interface.png`, `fig2_sites.png`, `fig3_extrapolation.png`. The follow-up in the last
section is not pre-registered (`results11/long/`).

Reproduce: `scripts/run_task11.py` (160 models, 15 min training + 11 min measurement on one GPU
and 96 CPU workers), `scripts/task11_followup.py` (10 min), `scripts/task11_tables.py`.
