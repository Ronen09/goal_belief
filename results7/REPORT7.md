...
pass over the saved weights (`scripts/task7_followup.py`, `results7/followup.json`).
Numbers from `results7/tables7.md`; per-run values, transform ratios, horizon
curves and sensitivity rows in `results7/results7.json`; weights in
`results7/models/`. Design: `docs/superpowers/specs/2026-09-18-representation-invariants-design.md`.
Reproduce with `scripts/run_task7.py --jobs 8` then `scripts/task7_followup.py`.
