...
this rebuilds `factor_runs` / `routing_runs` from `results10/models/*.pt`. Use it after a change
to the measurement code, or when a run crashed after training.

    .venv/bin/python scripts/task10_remeasure.py [--kind factor|routing|both]
"""
...
from run_task10 import DEFAULT_STEPS, LR, ROUTING, analyse_factor, analyse_routing, write_tables
...
def main():
    ...
    ap.add_argument("--out", default="results10"); ap.add_argument("--kind", default="both", choices=["factor", "routing", "both"])
