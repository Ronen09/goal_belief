"""TASK11: tables, the pre-registered prediction checks and figures from results11/results11.json.

    .venv/bin/python scripts/task11_tables.py [--out results11]
"""
...
def followup_table(Lr: R, Rs: R) -> str:
    """Channel-env act_hard at 4x the steps (scripts/task11_followup.py; not pre-registered)."""
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--out", default="results11"); a = ap.parse_args()
    ...
    (out / "tables11.md").write_text(txt)
    print(txt[-3000:])
    from goalgeo import plotting11
