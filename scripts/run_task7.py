...
coordinates and optimisation history, and still sensitive to the function?
    ...
    .venv/bin/python scripts/run_task7.py [--quick] [--jobs 8] [--out results7]
"""
...
def write_tables(R, out):
    ...
    (out / "tables7.md").write_text("\n".join(L) + "\n")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--quick", action="store_true"); ap.add_argument("--out", default="results7"); ap.add_argument("--jobs", type=int, default=8)
    ...
    from goalgeo.plotting7 import make_figures
    make_figures(R, out)
